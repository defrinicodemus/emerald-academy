...
import { AuthProvider, DEMO_ACCOUNTS, useAuth } from "@/lib/auth";
...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Masuk ke LMS SD Inpres Nggodimeda untuk murid, guru, kepala sekolah, dan admin." },
    ],
  }),
  component: () => (
    <AuthProvider>
...
function Login() {
  ...
  const submit = (uname?: string) => {
    setErr("");
    const u = login(uname ?? username);
    if (!u) {
      setErr("Akun tidak ditemukan. Coba salah satu akun demo di samping.");
      return;
    }
          ...
          <div className="rounded-3xl border bg-card p-6 shadow-soft md:p-8">
            ...
            >
              <div className="space-y-1.5">
                ...
                <Input id="u" placeholder="contoh: budi" value={username} onChange={(e) => setUsername(e.target.value)} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="p">Kata Sandi</Label>
                <Input id="p" type="password" placeholder="••••••••" value={pw} onChange={(e) => setPw(e.target.value)} />
              </div>
