...
export function ClassProvider({
  ...
  return (
    <Ctx.Provider value={{ active, activeClassId, setActive, classes }}>{children}</Ctx.Provider>
  );
}
...
export function ClassPicker() {
  ...
  return (
    <div className="flex items-center gap-2 rounded-full border bg-card px-3 py-1.5 text-card-foreground shadow-soft">
      <School className="h-4 w-4 text-primary" />
      <Select value={active ?? ""} onValueChange={(v) => setActive(v || null)}>
        <SelectTrigger className="h-7 w-[160px] border-0 bg-transparent p-0 text-sm font-medium text-card-foreground shadow-none focus:ring-0">
...
export function EmptyClassState({ message }: { message: string }) {
  return (
    <div className="grid place-items-center rounded-3xl border-2 border-dashed border-primary/30 bg-primary-soft/20 px-6 py-16 text-center">
      <div className="mb-4 grid h-16 w-16 place-items-center rounded-2xl bg-warning/20 text-3xl">
        ⚠️
      </div>
